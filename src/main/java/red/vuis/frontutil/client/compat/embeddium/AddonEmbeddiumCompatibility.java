...
public final class AddonEmbeddiumCompatibility {
	private static final AtomicBoolean REGISTERED = new AtomicBoolean();
	
	private AddonEmbeddiumCompatibility() {
	}
	...
	public static void init() {
		...
		BlockRendererRegistry.instance().registerRenderPopulator((renderers, context) -> {
			if (context.state().getBlock() instanceof BakedEntityBlock) {
				renderers.add(new BakedEntityBlockRenderer());
			}
		});
	}
	...
	private static final class BakedEntityBlockRenderer implements BlockRendererRegistry.Renderer {
